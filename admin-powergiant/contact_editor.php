...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_contact'])) {
        ...
        $image_path = null;
        if ($type === 'image' && !empty($_FILES['contact_image']['name'])) {
            $ext = strtolower(pathinfo($_FILES['contact_image']['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['contact_image']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'contact/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $file = 'contact_' . uniqid() . '.' . $ext;
                $target = $dir . $file;
                if (move_uploaded_file($_FILES['contact_image']['tmp_name'], $target)) {
                    $image_path = 'uploads/contact/' . $file;
                }
            }
        }

        try {
            $stmt = $db->prepare("INSERT INTO contact_content (section_name, content_type, content_value, image_path, display_order) VALUES (?,?,?,?,?)");
            $stmt->execute([$section, $type, $value, $image_path, $order]);
            $_SESSION['success_msg'] = "Contact section added!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: contact_editor.php"); exit();
    }
    ...
    if (isset($_POST['update_contact'])) {
        ...
        try {
            $stmt = $db->prepare("UPDATE contact_content SET section_name=?, content_type=?, content_value=?, image_path=?, display_order=? WHERE id=?");
            $stmt->execute([$section, $type, $value, $image_path, $order, $id]);
            $_SESSION['success_msg'] = "Contact section updated!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: contact_editor.php"); exit();
    ...
    if (isset($_POST['delete_contact'])) {
        $id = (int)$_POST['contact_id'];
        try {
            $stmt = $db->prepare("SELECT image_path FROM contact_content WHERE id=?");
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            if ($row && $row['image_path'] && file_exists('../' . $row['image_path'])) {
                unlink('../' . $row['image_path']);
            }
            $stmt = $db->prepare("DELETE FROM contact_content WHERE id=?");
            $stmt->execute([$id]);
            $_SESSION['success_msg'] = "Contact section deleted!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: contact_editor.php"); exit();
    ...
    if (isset($_POST['update_order'])) {
        ...
        $stmt = $db->prepare("UPDATE contact_content SET display_order=? WHERE id=?");
...
try {
    $sections = $db->query("SELECT * FROM contact_content ORDER BY display_order, id")->fetchAll();
} catch (PDOException $e) {
    $_SESSION['error_msg'] = "Error loading sections: " . $e->getMessage();
    $sections = [];
}
...
<head>
    ...
    <title>Contact Editor - <?= SITE_NAME ?></title>
            ...
            <div class="p-6 text-center">
                ...
                <div class="flex space-x-3">
                    ...
                    <form method="POST" id="deleteForm" class="flex-1">
                        ...
                        <button name="delete_contact" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">Delete</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-4 border-b bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800">Edit Contact Section</h3>
            ...
            <div class="p-4">
                <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-3">
                        <input type="text" name="section_name" id="edit_section" required placeholder="Section Name" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <select name="content_type" id="edit_type" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" onchange="toggleEditImage(this)">
                            <option value="text">Text</option>
                            <option value="image">Image</option>
                            <option value="contact_info">Contact Info</option>
                        </select>
                        <textarea name="content_value" id="edit_value" rows="4" required placeholder="Content Value" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"></textarea>
                    ...
                    <div class="space-y-3">
                        <div id="edit_image_upload" class="hidden">
                            <div id="currentImagePreview" class="mb-2"></div>
                            <input type="file" name="contact_image" accept="image/*" class="w-full px-3 py-2 border rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>

                        <!-- FIXED BUTTONS: Mobile-friendly, Red Cancel -->
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Contact Page Management</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="cards-container max-w-7xl mx-auto">
                <!-- Add Form -->
                <div class="contact-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Add Contact Section</h3>
                    <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        <input type="text" name="section_name" placeholder="Section Name (e.g., address)" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <select name="content_type" id="content_type" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" onchange="toggleImage(this)">
                            <option value="text">Text</option>
                            <option value="image">Image</option>
                            <option value="contact_info">Contact Info</option>
                        </select>
                        <textarea name="content_value" placeholder="Content (text, phone, email, etc.)" rows="4" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm resize-vertical min-h-[100px]"></textarea>
                        <div id="image_upload" class="hidden">
                            <input type="file" name="contact_image" accept="image/*" class="w-full px-4 py-3 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>
                        ...
                        <button name="add_contact" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition text-sm">Add Section</button>
                    </form>
                </div>

                <!-- List -->
                <div class="contact-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Contact Sections</h3>
                    <div id="contactList" class="space-y-3">
                        ...
                        <div data-id="<?= $s['id'] ?>" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:shadow-md drag-handle group">
                            <div class="flex items-center space-x-4 flex-1 w-full">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate text-sm sm:text-base"><?= htmlspecialchars($s['section_name']) ?></h4>
                                    <p class="text-xs text-gray-600 mt-1 mobile-line-clamp-2">
                                        <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs"><?= ucfirst(str_replace('_', ' ', $s['content_type'])) ?></span>
                                        <?= htmlspecialchars(strlen($s['content_value']) > 40 ? substr($s['content_value'], 0, 40) . '...' : $s['content_value']) ?>
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 mt-3 sm:mt-0 w-full sm:w-auto justify-end">
                                <button onclick='editSection(<?= json_encode($s, JSON_HEX_QUOT | JSON_HEX_APOS) ?>)' class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <p class="text-center text-gray-500 py-8">No contact sections yet. Add one to get started!</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function toggleImage(el) {
        document.getElementById('image_upload').classList.toggle('hidden', el.value !== 'image');
    }
    function toggleEditImage(el) {
        document.getElementById('edit_image_upload').classList.toggle('hidden', el.value !== 'image');
    }

    function editSection(s) {
        document.getElementById('edit_id').value = s.id;
        document.getElementById('edit_section').value = s.section_name;
        document.getElementById('edit_type').value = s.content_type;
        document.getElementById('edit_value').value = s.content_value;
        document.getElementById('edit_order').value = s.display_order;
        document.getElementById('edit_current_image').value = s.image_path || '';

        const prev = document.getElementById('currentImagePreview');
        prev.innerHTML = s.image_path 
            ? `<img src="../${s.image_path}" class="w-20 h-20 object-cover rounded-lg">` 
            : '<p class="text-gray-500 text-sm">No image</p>';

        toggleEditImage(document.getElementById('edit_type'));

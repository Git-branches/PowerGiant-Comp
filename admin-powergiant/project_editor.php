...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_project'])) {
        ...
        $cat   = trim($_POST['project_category']);
        ...
        if (!empty($_FILES['project_image']['name'])) {
            ...
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['project_image']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'projects/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $file = 'proj_' . uniqid() . '.' . $ext;
                $target = $dir . $file;
                if (move_uploaded_file($_FILES['project_image']['tmp_name'], $target)) {
                    // FIXED: Store relative path without "uploads/" prefix
                    $image_path = 'projects/' . $file;
                }
            }
        }

        try {
            $stmt = $db->prepare("INSERT INTO projects_content (project_title, project_category, project_description, client_name, project_date, image_path, display_order) VALUES (?,?,?,?,?,?,?)");
            $stmt->execute([$title, $cat, $desc, $client, $date, $image_path, $order]);
            $_SESSION['success_msg'] = "Project added!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: project_editor.php"); exit();
    }
    ...
    if (isset($_POST['update_project'])) {
        ...
        try {
            $stmt = $db->prepare("UPDATE projects_content SET project_title=?, project_category=?, project_description=?, client_name=?, project_date=?, image_path=?, display_order=? WHERE id=?");
            $stmt->execute([$title, $cat, $desc, $client, $date, $image_path, $order, $id]);
    ...
    if (isset($_POST['delete_project'])) {
        $id = (int)$_POST['project_id'];
        try {
            $stmt = $db->prepare("SELECT image_path FROM projects_content WHERE id=?");
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            ...
            $stmt = $db->prepare("DELETE FROM projects_content WHERE id=?");
            $stmt->execute([$id]);
    ...
    if (isset($_POST['update_order'])) {
        ...
        $stmt = $db->prepare("UPDATE projects_content SET display_order=? WHERE id=?");
...
try {
    $projects = $db->query("SELECT * FROM projects_content ORDER BY display_order, id")->fetchAll();
} catch (PDOException $e) {
    $_SESSION['error_msg'] = "Error loading projects: " . $e->getMessage();
    $projects = [];
}
...
<head>
    ...
    <title>Projects Editor - <?= SITE_NAME ?></title>
...
<div class="flex h-screen overflow-hidden">
            ...
            <div class="p-6 text-center">
                ...
                <div class="flex space-x-3">
                    ...
                    <form method="POST" id="deleteForm" class="flex-1">
                        ...
                        <button name="delete_project" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">Delete</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-4">
                <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-3">
                        <input type="text" name="project_title" id="edit_title" required placeholder="Title" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="text" name="project_category" id="edit_cat" required placeholder="Category" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="text" name="client_name" id="edit_client" required placeholder="Client" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="date" name="project_date" id="edit_date" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    ...
                    <div class="space-y-3">
                        <textarea name="project_description" id="edit_desc" rows="4" required placeholder="Description" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"></textarea>
                        <div id="edit_image_upload">
                            <div id="currentImagePreview" class="mb-2"></div>
                            <input type="file" name="project_image" accept="image/*" class="w-full px-3 py-2 border rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>

                        <!-- FIXED BUTTONS: Red Cancel First on Mobile -->
    ...
    <!-- Sidebar (Same as contact_editor.php) -->
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Projects Management</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="cards-container max-w-7xl mx-auto">
                <!-- Add Form -->
                <div class="project-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Add New Project</h3>
                    <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        <input type="text" name="project_title" placeholder="Title" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="text" name="project_category" placeholder="Category" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="text" name="client_name" placeholder="Client" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="date" name="project_date" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <textarea name="project_description" placeholder="Description" rows="4" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm resize-vertical min-h-[100px]"></textarea>
                        <div id="image_upload">
                            <input type="file" name="project_image" accept="image/*" class="w-full px-4 py-3 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>
                        <input type="number" name="display_order" value="0" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <button name="add_project" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition text-sm">Add Project</button>
                    </form>
                </div>

                <!-- List -->
                <div class="project-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Existing Projects</h3>
                    <div id="projectList" class="space-y-3">
                        <?php foreach ($projects as $p): ?>
                        <div data-id="<?= $p['id'] ?>" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:shadow-md drag-handle group">
                            <div class="flex items-center space-x-4 flex-1 w-full">
                                <svg class="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                                </svg>
                                <?php if ($p['image_path']): 
                                    // FIXED: Use proper path for preview
                                    $full_image_path = "../uploads/" . $p['image_path'];
                                    if (file_exists($full_image_path)): ?>
                                    <img src="<?= $full_image_path ?>" class="w-12 h-12 object-cover rounded-lg flex-shrink-0" alt="">
                                <?php else: ?>
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate text-sm sm:text-base"><?= htmlspecialchars($p['project_title']) ?></h4>
                                    <p class="text-xs text-gray-600 mt-1 mobile-line-clamp-2">
                                        <?= htmlspecialchars(strlen($p['project_description']) > 40 ? substr($p['project_description'], 0, 40) . '...' : $p['project_description']) ?>
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 mt-3 sm:mt-0 w-full sm:w-auto mobile-justify-end">
                                <button onclick='editProject(<?= json_encode($p, JSON_HEX_QUOT | JSON_HEX_APOS) ?>)' class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg transition mobile-min-w-11 mobile-min-h-11">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button onclick='openDeleteModal(<?= $p['id'] ?>)' class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition mobile-min-w-11 mobile-min-h-11">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </button>
                        ...
                        <p class="text-center text-gray-500 py-8">No projects yet. Add one to get started!</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function editProject(p) {
        ...
        document.getElementById('edit_title').value = p.project_title;
        document.getElementById('edit_cat').value = p.project_category;
        document.getElementById('edit_desc').value = p.project_description;
        document.getElementById('edit_client').value = p.client_name;
        document.getElementById('edit_date').value = p.project_date;
        document.getElementById('edit_order').value = p.display_order;
        document.getElementById('edit_current_image').value = p.image_path || '';

        const prev = document.getElementById('currentImagePreview');
        // FIXED: Use proper path for preview in modal
        if (p.image_path) {
            const fullImagePath = "../uploads/" + p.image_path;
            prev.innerHTML = `<img src="${fullImagePath}" class="w-20 h-20 object-cover rounded-lg">`;
        } else {
            prev.innerHTML = '<p class="text-gray-500 text-sm">No image</p>';
        }

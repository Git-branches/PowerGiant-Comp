...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['csrf_token'], $token)) {
        ...
        header("Location: compliance_editor.php"); exit();
    }

    // ADD CERTIFICATE
    if (isset($_POST['add_compliance'])) {
        $name = trim($_POST['certificate_name']);
        $desc = trim($_POST['certificate_description']);
        ...
        // Auto display order
        $stmt = $db->query("SELECT MAX(display_order) as max_order FROM compliance_content");
        $max = $stmt->fetch();
        $order = ($max['max_order'] ?? 0) + 1;

        $cert_file = $image_path = null;

        // Upload PDF/DOC
        if (!empty($_FILES['certificate_file']['name']) && $_FILES['certificate_file']['error'] === UPLOAD_ERR_OK) {
            $ext = strtolower(pathinfo($_FILES['certificate_file']['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ['pdf', 'doc', 'docx']) && $_FILES['certificate_file']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'compliance/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $file = 'cert_' . uniqid() . '.' . $ext;
                $target = $dir . $file;
                if (move_uploaded_file($_FILES['certificate_file']['tmp_name'], $target)) {
                    $cert_file = 'uploads/compliance/' . $file;
                } else {
                    $_SESSION['error_msg'] = "Failed to upload certificate file.";
                    header("Location: compliance_editor.php"); exit();
                }
            } else {
                $_SESSION['error_msg'] = "Invalid file type or size (PDF/DOC/DOCX, max " . (MAX_FILE_SIZE / 1024 / 1024) . "MB).";
                header("Location: compliance_editor.php"); exit();
            }
        }
        ...
        if (!empty($_FILES['certificate_image']['name']) && $_FILES['certificate_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['certificate_image']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'compliance/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $file = 'img_' . uniqid() . '.' . $ext;
                $target = $dir . $file;
                if (move_uploaded_file($_FILES['certificate_image']['tmp_name'], $target)) {
                    $image_path = 'uploads/compliance/' . $file;
                } else {
                    $_SESSION['error_msg'] = "Failed to upload image.";
                    header("Location: compliance_editor.php"); exit();
                }
            } else {
                $_SESSION['error_msg'] = "Invalid image type or size (max " . (MAX_FILE_SIZE / 1024 / 1024) . "MB).";
                header("Location: compliance_editor.php"); exit();
            }
        }

        try {
            $stmt = $db->prepare("INSERT INTO compliance_content (certificate_name, certificate_description, issuing_authority, issue_date, expiry_date, certificate_file, image_path, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $desc, $authority, $issue, $expiry, $cert_file, $image_path, $order, $active]);
            $_SESSION['success_msg'] = "Certificate added successfully!";
    ...
    if (isset($_POST['update_compliance'])) {
        ...
        $active = isset($_POST['is_active']) ? 1 : 0;
        $current_file = $_POST['current_certificate'] ?? '';
        $current_image = $_POST['current_image'] ?? '';

        $cert_file = $current_file;
        ...
        if (!empty($_FILES['certificate_file']['name']) && $_FILES['certificate_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ['pdf','doc','docx']) && $_FILES['certificate_file']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'compliance/';
                $file = 'cert_' . uniqid() . '.' . $ext;
                if (move_uploaded_file($_FILES['certificate_file']['tmp_name'], $dir . $file)) {
                    $cert_file = 'uploads/compliance/' . $file;
                }
            }
        }
        ...
        if (!empty($_FILES['certificate_image']['name']) && $_FILES['certificate_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['certificate_image']['size'] <= MAX_FILE_SIZE) {
                $dir = UPLOAD_DIR . 'compliance/';
                $file = 'img_' . uniqid() . '.' . $ext;
                if (move_uploaded_file($_FILES['certificate_image']['tmp_name'], $dir . $file)) {
                    $image_path = 'uploads/compliance/' . $file;
                }
            }
        }
        ...
        try {
            $stmt = $db->prepare("UPDATE compliance_content SET certificate_name=?, certificate_description=?, issuing_authority=?, issue_date=?, expiry_date=?, certificate_file=?, image_path=?, display_order=?, is_active=? WHERE id=?");
            $stmt->execute([$name, $desc, $authority, $issue, $expiry, $cert_file, $image_path, $order, $active, $id]);
    ...
    if (isset($_POST['delete_compliance'])) {
        $id = (int)$_POST['compliance_id'];
        try {
            $stmt = $db->prepare("SELECT certificate_file, image_path FROM compliance_content WHERE id=?");
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            if ($row) {
                if ($row['certificate_file'] && file_exists('../' . $row['certificate_file'])) unlink('../' . $row['certificate_file']);
                if ($row['image_path'] && file_exists('../' . $row['image_path'])) unlink('../' . $row['image_path']);
            }
            $stmt = $db->prepare("DELETE FROM compliance_content WHERE id=?");
            $stmt->execute([$id]);
    ...
    if (isset($_POST['update_order'])) {
        ...
        if (is_array($order)) {
            $stmt = $db->prepare("UPDATE compliance_content SET display_order=? WHERE id=?");
...
try {
    $certs = $db->query("SELECT * FROM compliance_content ORDER BY display_order ASC, issue_date DESC")->fetchAll();
} catch (PDOException $e) {
    $_SESSION['error_msg'] = "Error loading certificates: " . $e->getMessage();
    $certs = [];
}
...
<head>
    ...
    <title>Compliance Editor - <?= SITE_NAME ?></title>
    ...
    <style>
        ...
        .modal-action-btn {
            @apply h-11 px-4 flex items-center justify-center text-sm font-medium rounded-lg transition w-full;
        }
        ...
        @media (max-width: 768px) {
            ...
            .mobile-items  { align-items: flex-start; }
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">Delete Certificate</h3>
                <p class="text-gray-600 mb-6">This action cannot be undone.</p>
                <div class="flex space-x-3 mobile:flex-col mobile:space-y-3 mobile:space-x-0">
                    <button onclick="closeDeleteModal()" class="modal-action-btn border border-gray-300 text-gray-700 hover:bg-gray-50">
                        Cancel
                    </button>
                    <form method="POST" id="deleteForm" class="flex-1">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                        <input type="hidden" name="compliance_id" id="delete_compliance_id">
                        <button name="delete_compliance" class="modal-action-btn bg-red-600 hover:bg-red-700 text-white">
                            Delete
                        </button>
                    </form>
                </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-4 border-b bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800">Edit Certificate</h3>
            ...
            <div class="p-4">
                <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                    <input type="hidden" name="compliance_id" id="edit_id">
                    <input type="hidden" name="current_certificate" id="edit_current_file">
                    <input type="hidden" name="current_image" id="edit_current_image">

                    <div class="space-y-3">
                        <input type="text" name="certificate_name" id="edit_name" required placeholder="Certificate Name" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="text" name="issuing_authority" id="edit_authority" required placeholder="Issuing Authority" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="date" name="issue_date" id="edit_issue" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <input type="date" name="expiry_date" id="edit_expiry" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                    ...
                    <div class="space-y-3">
                        <textarea name="certificate_description" id="edit_desc" rows="4" required placeholder="Description" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"></textarea>
                        <div id="currentFilePreview" class="mb-2"></div>
                        <input type="file" name="certificate_file" accept=".pdf,.doc,.docx" class="w-full px-3 py-2 border rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        <div id="currentImagePreview" class="mb-2"></div>
                        <input type="file" name="certificate_image" accept="image/*" class="w-full px-3 py-2 border rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="is_active" id="edit_active" class="w-4 h-4 text-blue-600 rounded">
                            <span class="text-sm font-medium text-gray-700">Active</span>
                        </label>
                    </div>

                    <div class="md:col-span-2 flex space-x-3 pt-2 mobile:flex-col mobile:space-y-3 mobile:space-x-0">
                        <button type="button" onclick="closeEditModal()" class="flex-1 h-11 px-4 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition text-sm flex items-center justify-center mobile:w-full">
                            Cancel
                        </button>
                        <button name="update_compliance" class="flex-1 h-11 px-4 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition text-sm flex items-center justify-center mobile:w-full">
                            Update Certificate
                        </button>
                    </div>
                </form>
            ...
            <div class="p-6">
                <form id="changePasswordForm" method="POST" action="change_password.php" class="space-y-4">
                    ...
                    <div class="flex flex-col md:flex-row gap-3 pt-4">
                        <button type="button" onclick="closeSettingsModal()" class="w-full md:w-auto px-4 py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50">
                            Cancel
                        </button>
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Compliance Management</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            <?php if ($success_msg): ?>
                <div id="successMessage" class="toast-notification bg-green-50 border-l-4 border-green-500 text-gray-800 px-6 py-4 rounded-lg shadow-xl flex items-center">
                    ...
                    <div class="flex-1"><p class="font-semibold text-green-800">Success!</p><p class="text-sm text-gray-700"><?= htmlspecialchars($success_msg) ?></p></div>
            ...
            <?php if ($error_msg): ?>
                <div id="errorMessage" class="toast-notification bg-red-50 border-l-4 border-red-500 text-gray-800 px-6 py-4 rounded-lg shadow-xl flex items-center">
                    ...
                    <div class="flex-1"><p class="font-semibold text-red-800">Error!</p><p class="text-sm text-gray-700"><?= htmlspecialchars($error_msg) ?></p></div>
                    <button onclick="this.parentElement.remove()" class="ml-4 text-gray-400 hover:text-gray-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>
            ...
            <div class="cards-container max-w-7xl mx-auto">
                <!-- Add Form -->
                <div class="compliance-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Add Certificate</h3>
                    <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                        <input type="text" name="certificate_name" placeholder="Certificate Name" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <textarea name="certificate_description" placeholder="Description" rows="3" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm resize-vertical min-h-[80px]"></textarea>
                        <input type="text" name="issuing_authority" placeholder="Issuing Authority" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input type="date" name="issue_date" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                            <input type="date" name="expiry_date" placeholder="Expiry (Optional)" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        </div>
                        <input type="file" name="certificate_file" accept=".pdf,.doc,.docx" class="w-full px-4 py-3 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        <input type="file" name="certificate_image" accept="image/*" class="w-full px-4 py-3 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="is_active" checked class="w-4 h-4 text-blue-600 rounded">
                            <span class="text-sm font-medium text-gray-700">Active</span>
                        </label>
                        <button name="add_compliance" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition text-sm">Add Certificate</button>
                    </form>
                </div>

                <!-- List -->
                <div class="compliance-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Certificates</h3>
                    <div id="certList" class="space-y-3">
                        <?php foreach ($certs as $c): ?>
                        <div data-id="<?= $c['id'] ?>" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:shadow-md drag-handle group">
                            <div class="flex items-center space-x-4 flex-1 w-full">
                                <svg class="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                                </svg>
                                <?php if ($c['image_path']): ?>
                                    <img src="../<?= htmlspecialchars($c['image_path'] ?? '') ?>" class="w-12 h-12 object-cover rounded-lg flex-shrink-0" alt="<?= htmlspecialchars($c['certificate_name'] ?? 'Certificate') ?>">
                                <?php else: ?>
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate text-sm sm:text-base"><?= htmlspecialchars($c['certificate_name'] ?? '') ?></h4>
                                    <p class="text-xs text-gray-600 mt-1 mobile-line-clamp-2">
                                        <?= htmlspecialchars($c['issuing_authority'] ?? '') ?> • <?= date('M d, Y', strtotime($c['issue_date'] ?? 'now')) ?>
                                        <?php if ($c['certificate_file']): ?>
                                            <br><a href="../<?= htmlspecialchars($c['certificate_file'] ?? '') ?>" target="_blank" class="text-blue-600 hover:underline text-xs">View File</a>
                                        <?php endif; ?>
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 mt-3 sm:mt-0 w-full sm:w-auto justify-end">
                                <span class="px-3 py-1 text-xs font-medium rounded-full <?= $c['is_active'] ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' ?> whitespace-nowrap">
                                    <?= $c['is_active'] ? 'Active' : 'Inactive' ?>
                                </span>
                                <button onclick='editCert(<?= json_encode($c, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>)' class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button onclick='openDeleteModal(<?= $c['id'] ?>)' class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </button>
                        ...
                        <?php endforeach; ?>
                        <?php if (empty($certs)): ?>
                        <p class="text-center text-gray-500 py-8">No certificates yet. Add one to get started!</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function togglePageManagement() {
        ...
        dropdown.classList.toggle('py-0');
    ...
    function editCert(c) {
        ...
        document.getElementById('edit_name').value = c.certificate_name;
        document.getElementById('edit_desc').value = c.certificate_description;
        document.getElementById('edit_authority').value = c.issuing_authority;
        document.getElementById('edit_issue').value = c.issue_date;
        document.getElementById('edit_expiry').value = c.expiry_date || '';
        document.getElementById('edit_order').value = c.display_order;
        document.getElementById('edit_active').checked = c.is_active == 1;
        document.getElementById('edit_current_file').value = c.certificate_file || '';
        document.getElementById('edit_current_image').value = c.image_path || '';

        document.getElementById('currentFilePreview').innerHTML = c.certificate_file 
            ? `<a href="../${c.certificate_file}" target="_blank" class="text-blue-600 hover:underline text-sm">Current: ${c.certificate_file.split('/').pop()}</a>` 
            : '<p class="text-gray-500 text-sm">No file</p>';

        document.getElementById('currentImagePreview').innerHTML = c.image_path 
            ? `<img src="../${c.image_path}" class="w-20 h-20 object-cover rounded-lg" alt="Current Image">` 
    ...
    function openDeleteModal(id) {
        document.getElementById('delete_compliance_id').value = id;

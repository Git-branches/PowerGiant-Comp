...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_service'])) {
        ...
        if (!empty($_FILES['service_image']['name'])) {
            ...
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['service_image']['size'] <= MAX_FILE_SIZE) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO services_content (service_name, service_description, service_icon, image_path, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$service_name, $service_description, $service_icon, $image_path, $display_order, $is_active]);
            $_SESSION['success_msg'] = "Service added!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: service_editor.php"); exit();
    }
    ...
    if (isset($_POST['update_service'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE services_content SET service_name=?, service_description=?, service_icon=?, image_path=?, display_order=?, is_active=?, updated_at=CURRENT_TIMESTAMP WHERE id=?");
            $stmt->execute([$service_name, $service_description, $service_icon, $image_path, $display_order, $is_active, $id]);
            $_SESSION['success_msg'] = "Service updated!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: service_editor.php"); exit();
    ...
    if (isset($_POST['delete_service'])) {
        $id = (int)$_POST['service_id'];
        try {
            $stmt = $pdo->prepare("SELECT image_path FROM services_content WHERE id=?");
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            ...
            $stmt->execute([$id]);
            $_SESSION['success_msg'] = "Service deleted!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Error: " . $e->getMessage();
        }
        header("Location: service_editor.php"); exit();
    ...
    if (isset($_POST['update_order'])) {
        ...
        if (is_array($order)) {
            $stmt = $pdo->prepare("UPDATE services_content SET display_order=? WHERE id=?");
...
// Fetch services
$services = $pdo->query("SELECT * FROM services_content ORDER BY display_order, id")->fetchAll();
...
$admin_name = htmlspecialchars($_SESSION['admin_name'] ?? 'Admin', ENT_QUOTES);
?>
...
<head>
    ...
    <title>Services Editor - <?= SITE_NAME ?></title>
    ...
    <style>
        ...
        #sidebarOverlay.show, #modalOverlay.show, #deleteModalOverlay.show, #settingsModalOverlay.show { display:block; animation:fadeIn .3s; }

        #sidebar { transition:transform .3s ease-in-out; }
        #editModal, #deleteModal, #settingsModal { transition:all .3s; transform:scale(.95); opacity:0; }
        #editModal.show, #deleteModal.show, #settingsModal.show { transform:scale(1); opacity:1; }

        @keyframes fadeIn { from{opacity:0} to{opacity:1} }

        .drag-handle { cursor:move; }
        .dragging { opacity:.5; background:#dbeafe; }
        .icon-preview i { font-size:1.5rem; }
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">Delete Service</h3>
                <p class="text-gray-600 mb-6">Are you sure? This cannot be undone.</p>
                <div class="flex space-x-3">
                    <button onclick="closeDeleteModal()" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50">
                        Cancel
                    </button>
                    <form method="POST" class="flex-1">
                        <input type="hidden" name="service_id" id="delete_service_id">
                        <button name="delete_service" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg">
                            Delete
                        </button>
                    </form>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-4">
                <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-3">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Service Name</label>
                            <input type="text" name="service_name" id="edit_name" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Icon Class</label>
                            <div class="flex items-center space-x-2">
                                <input type="text" name="service_icon" id="edit_icon" placeholder="fas fa-truck" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                                <div class="icon-preview text-blue-600"><i class="fas fa-cog"></i></div>
                            </div>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Display Order</label>
                            <input type="number" name="display_order" id="edit_order" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        </div>
                        <div class="flex items-center space-x-2">
                            <input type="checkbox" name="is_active" id="edit_active" class="w-4 h-4 text-blue-600 rounded focus:ring-blue-500">
                    ...
                    <div class="space-y-3">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea name="service_description" id="edit_desc" rows="4" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"></textarea>
                        </div>
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">New Image</label>
                            <input type="file" name="service_image" accept="image/*" class="w-full px-3 py-2 border border-gray-300 rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div class="p-6">
                <form id="changePasswordForm" method="POST" action="change_password.php" class="space-y-4">
                    ...
                    <div class="flex space-x-3 pt-4 mobile-stack mobile-space-y-4">
                        <button type="button" onclick="closeSettingsModal()" class="flex-1 px-4 py-3 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 mobile-full">
                            Cancel
                        </button>
                        <button type="submit" id="submitPassword" class="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg flex items-center justify-center mobile-full">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Services Management</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="cards-container max-w-7xl mx-auto">
                <!-- Add Form -->
                <div class="service-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Add New Service</h3>
                    <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        <input type="text" name="service_name" placeholder="Service Name" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <textarea name="service_description" placeholder="Description" rows="4" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm resize-vertical min-h-[100px]"></textarea>
                        <div class="flex items-center space-x-2">
                            <input type="text" name="service_icon" placeholder="fas fa-truck" class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                            <div class="icon-preview text-blue-600"><i class="fas fa-truck text-lg"></i></div>
                        </div>
                        <input type="file" name="service_image" accept="image/*" class="w-full px-4 py-3 border border-gray-300 rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        <input type="number" name="display_order" value="0" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" checked class="w-4 h-4 text-blue-600 rounded focus:ring-blue-500">
                            <label class="ml-2 text-sm font-medium text-gray-700">Active</label>
                        </div>
                        <button name="add_service" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition mt-2 text-sm">
                            Add Service
                        </button>
                    </form>
                </div>
                ...
                <!-- List -->
                <div class="service-card bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4 border-b pb-3">Existing Services</h3>
                    <div id="servicesList" class="space-y-3 flex-1 overflow-y-auto">
                        <?php foreach ($services as $s): 
                            $desc = (strlen($s['service_description'] ?? '') > 60) ? substr($s['service_description'] ?? '', 0, 60).'...' : ($s['service_description'] ?? '');
                            $icon = $s['service_icon'] ?? 'fas fa-cog';
                            $name = $s['service_name'] ?? 'Untitled';
                        ?>
                        <div data-id="<?= $s['id'] ?>" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:shadow-md drag-handle group">
                            <div class="flex items-center space-x-4 flex-1 w-full">
                                <svg class="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                                </svg>
                                <?php if (!empty($s['image_path']) && file_exists('../'.$s['image_path'])): ?>
                                    <img src="../<?= htmlspecialchars($s['image_path']) ?>" class="w-12 h-12 object-cover rounded-lg flex-shrink-0" alt="">
                                <?php else: ?>
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate text-sm sm:text-base"><?= htmlspecialchars($name) ?></h4>
                                    <p class="text-xs text-gray-600 mt-1"><?= htmlspecialchars($desc) ?></p>
                                </div>
                                <div class="icon-preview text-blue-600 ml-4 flex-shrink-0"><i class="<?= htmlspecialchars($icon) ?> text-lg"></i></div>
                            </div>
                            <div class="flex items-center gap-2 mt-3 sm:mt-0 w-full sm:w-auto justify-end sm:justify-start">
                                <span class="px-3 py-1 text-xs font-medium rounded-full <?= $s['is_active'] ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' ?> whitespace-nowrap">
                                    <?= $s['is_active'] ? 'Active' : 'Inactive' ?>
                                </span>
                                <button onclick='editService(<?= json_encode($s, JSON_HEX_QUOT | JSON_HEX_APOS) ?>)' class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button onclick='openDeleteModal(<?= $s['id'] ?>)' class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </button>
                        ...
                        <p class="text-center text-gray-500 py-8">No services yet.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function togglePageManagement() {
        ...
        dropdown.classList.toggle('max-h-0'); dropdown.classList.toggle('py-0');
        dropdown.classList.toggle('max-h-96'); dropdown.classList.toggle('py-2');
    ...
    document.addEventListener('click', e => {
        ...
        if (!e.target.closest('button[onclick="toggleUserDropdown()"]') && !dd.classList.contains('hidden')) dd.classList.add('hidden');
    });
    ...
    // Icon Preview
    document.querySelectorAll('input[name="service_icon"]').forEach(i => {
        i.addEventListener('input', () => {
            const preview = i.closest('div').querySelector('.icon-preview i');
            preview.className = i.value.trim() || 'fas fa-cog';
        });
    });
    ...
    function editService(s) {
        ...
        document.getElementById('edit_name').value = s.service_name || '';
        document.getElementById('edit_desc').value = s.service_description || '';
        document.getElementById('edit_icon').value = s.service_icon || '';
        document.getElementById('edit_order').value = s.display_order || 0;
        document.getElementById('edit_active').checked = s.is_active == 1;
        document.getElementById('edit_current_image').value = s.image_path || '';

        const preview = document.getElementById('currentImagePreview');
        preview.innerHTML = s.image_path 
            ? `<img src="../${s.image_path}" class="w-20 h-20 object-cover rounded-lg">` 
            : '<p class="text-gray-500">No image</p>';

        const iconInput = document.getElementById('edit_icon');
        const iconPrev = iconInput.closest('div').querySelector('.icon-preview i');
        iconPrev.className = s.service_icon || 'fas fa-cog';

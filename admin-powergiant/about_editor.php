...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_about'])) {
        ...
        $image_path = null;
        if ($type === 'image' && !empty($_FILES['about_image']['name'])) {
            $ext = strtolower(pathinfo($_FILES['about_image']['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ALLOWED_FILE_TYPES) && $_FILES['about_image']['size'] <= MAX_FILE_SIZE) {
                $dir = '../uploads/about/';
                if (!is_dir($dir)) mkdir($dir, 0755, true);
                $file = 'about_' . uniqid() . '.' . $ext;
                $target = $dir . $file;
                if (move_uploaded_file($_FILES['about_image']['tmp_name'], $target)) {
                    $image_path = 'uploads/about/' . $file;
                }
            }
        }

        try {
            $stmt = $db->prepare("INSERT INTO about_content 
                (section_name, content_type, content_value, image_path, display_order) 
                VALUES (?,?,?,?,?)");
            $stmt->execute([$section, $type, $value, $image_path, $order]);
            $_SESSION['success_msg'] = "About section added!";
        } catch (PDOException $e) {
            $_SESSION['error_msg'] = "Add failed: " . $e->getMessage();
        }
        header("Location: about_editor.php"); exit();
    }
    ...
    if (isset($_POST['update_about'])) {
        ...
        if ($type === 'image' && !empty($_FILES['about_image']['name'])) {
            ...
            // If changing from image to text, delete the image
            if ($current && file_exists('../' . $current)) {
                @unlink('../' . $current);
            }
            $image_path = null;
        ...
        try {
            $stmt = $db->prepare("UPDATE about_content SET 
                section_name=?, content_type=?, content_value=?, image_path=?, display_order=?, updated_at=CURRENT_TIMESTAMP 
                WHERE id=?");
            $stmt->execute([$section, $type, $value, $image_path, $order, $id]);
            $_SESSION['success_msg'] = "About section updated!";
        } catch (PDOException $e) {
    ...
    if (isset($_POST['delete_about'])) {
        $id = (int)$_POST['about_id'];
        try {
            $stmt = $db->prepare("SELECT image_path FROM about_content WHERE id=?");
            $stmt->execute([$id]);
            $row = $stmt->fetch();
            ...
            $stmt->execute([$id]);
            $_SESSION['success_msg'] = "About section deleted!";
        } catch (PDOException $e) {
    ...
    if (isset($_POST['update_order'])) {
        ...
        if (is_array($order)) {
            $stmt = $db->prepare("UPDATE about_content SET display_order=? WHERE id=?");
...
/* ────────────────────── FETCH ────────────────────── */
try {
    $sections = $db->query("SELECT * FROM about_content ORDER BY display_order, id")->fetchAll();
} catch (PDOException $e) {
...
<head>
    ...
    <title>About Editor - <?= SITE_NAME ?></title>
            ...
            <div class="p-6 text-center">
                ...
                <div class="flex space-x-3">
                    ...
                    <form method="POST" id="deleteForm" class="flex-1">
                        <input type="hidden" name="about_id" id="delete_about_id">
                        <button name="delete_about" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">Delete</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-4 border-b bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800">Edit About Section</h3>
            ...
            <div class="p-4">
                <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="hidden" name="about_id" id="edit_id">
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="content_value" id="edit_value" rows="4" required placeholder="Content" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 text-sm"></textarea>
                    ...
                    <div class="space-y-3">
                        <div id="edit_image_upload" class="hidden">
                            ...
                            <input type="file" name="about_image" accept="image/*" class="w-full px-3 py-2 border rounded-lg file:mr-3 file:py-1.5 file:px-3 file:rounded-full file:bg-blue-50 file:text-blue-700 text-sm">
                        </div>
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">About Page Management</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="cards-container max-w-7xl mx-auto">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Add About Section</h3>
                    <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        <input type="text" name="section_name" placeholder="Section Name" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                        <select name="content_type" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500" onchange="toggleImage(this)">
                            <option value="text">Text</option>
                            <option value="image">Image</option>
                        </select>
                        <textarea name="content_value" placeholder="Content" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500"></textarea>
                        <div id="image_upload" class="hidden">
                            <input type="file" name="about_image" accept="image/*" class="w-full px-4 py-3 border rounded-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-blue-50 file:text-blue-700">
                        </div>
                        ...
                        <button name="add_about" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition">Add Section</button>
                    </form>
                </div>

                <!-- List -->
                <div class="bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">About Sections</h3>
                    <div id="aboutList" class="space-y-3">
                        <?php foreach ($sections as $s): 
                            $content = $s['content_value'] ?? '';
                            $preview = (strlen($content) > 40) ? substr($content, 0, 40) . '...' : $content;
                        ?>
                        <div data-id="<?= $s['id'] ?>" class="flex items-center justify-between p-4 border rounded-lg hover:shadow-md drag-handle group mobile-p-3 mobile-flex-col mobile-items-start">
                            <div class="flex items-center gap-3 flex-1 min-w-0 mobile-w-full">
                                ...
                                <?php if ($s['content_type'] === 'image' && $s['image_path']): ?>
                                    <img src="../<?= htmlspecialchars($s['image_path']) ?>" class="w-12 h-12 object-cover rounded-lg flex-shrink-0" alt="" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                    <div class="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0 hidden">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate mobile-text-xs"><?= htmlspecialchars($s['section_name'] ?? '') ?></h4>
                                    <p class="text-xs text-gray-600 mobile-line-clamp-2">
                                    <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs"><?= ucfirst($s['content_type'] ?? 'text') ?></span>
                                    <?= htmlspecialchars($preview) ?>
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center gap-1 mobile-gap-1 mobile-mt-1 mobile-justify-end mobile-w-full">
                                <button onclick='editSection(<?= json_encode($s, JSON_HEX_QUOT | JSON_HEX_APOS) ?>)' class="p-2.5 text-yellow-600 hover:bg-yellow-50 rounded-lg transition mobile-min-w-11 mobile-min-h-11">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button onclick='openDeleteModal(<?= $s['id'] ?>)' class="p-2.5 text-red-600 hover:bg-red-50 rounded-lg transition mobile-min-w-11 mobile-min-h-11">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </button>
                        ...
                        <p class="text-center text-gray-500 py-8">No about sections yet. Add one to get started!</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
function editSection(s) {
    ...
    document.getElementById('edit_type').value = s.content_type || 'text';
    ...
    if (s.image_path) {
        preview.innerHTML = `<img src="../${s.image_path}" class="w-20 h-20 object-cover rounded-lg" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';"><p class="text-gray-500 text-sm hidden">Image not found</p>`;
    } else {
        preview.innerHTML = '<p class="text-gray-500 text-sm">No image</p>';
    }

...
requirePermission('backup_maintenance');
$database = new Database();
$conn = $database->connect();
$dbname = 'powergiant_quotes';
...
function backupDatabase($conn, $dbname) {
    ...
    $output = "-- =======================================\n";
    $output .= "-- Database Backup: " . SITE_NAME . "\n";
        ...
        if ($stmt->rowCount() > 0) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
                ...
                foreach ($row as $i => $value) {
                    if ($i > 0) $output .= ", ";
                    $output .= $value === null ? "NULL" : $conn->quote($value);
                }
                $output .= ")";
                $first = false;
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    switch ($_POST['action']) {
        case 'backup':
            try {
                $filename = backupDatabase($conn, $dbname);
                $message = "Backup created: <strong>$filename</strong>";
                $message_type = 'success';
            } catch (Exception $e) {
                $message = "Backup failed: " . htmlspecialchars($e->getMessage());
                $message_type = 'error';
            }
            break;
        ...
        case 'optimize':
            ...
            $message = "Optimized $optimized_count table" . ($optimized_count != 1 ? 's' : '') . " successfully!";
            $message_type = 'success';
            break;
        ...
        case 'clean_logs':
            ...
            $message = "Deleted $affected log entr" . ($affected != 1 ? 'ies' : 'y') . " older than $days days.";
            $message_type = 'success';
            break;

        case 'delete_backup':
            $filename = basename($_POST['filename']);
            if (preg_match('/^backup_' . preg_quote($dbname, '/') . '_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.sql$/', $filename)) {
                $filepath = '../backups/' . $filename;
                if (file_exists($filepath)) {
                    unlink($filepath);
                    $message = "Backup deleted: <strong>$filename</strong>";
                    $message_type = 'success';
                } else {
...
// ========================================
$stats = [];
$stmt = $conn->query("SELECT COUNT(*) FROM quote_requests"); $stats['quotes'] = $stmt->fetchColumn();
$stmt = $conn->query("SELECT COUNT(*) FROM admin_user"); $stats['users'] = $stmt->fetchColumn();
$stmt = $conn->query("SELECT COUNT(*) FROM activity_logs"); $stats['logs'] = $stmt->fetchColumn();
...
<head>
    ...
    <title>Backup & Maintenance - <?= SITE_NAME ?></title>
...
<div class="flex h-screen overflow-hidden">
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-2">Delete Backup</h3>
                ...
                <div class="flex space-x-3">
                    ...
                    <form method="POST" id="deleteForm" class="flex-1">
                        ...
                        <button type="submit" class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">Delete</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <!-- Sidebar - 100% IDENTICAL TO news_editor.php -->
                    ...
                    <div id="pageManagementDropdown" class="max-h-0 overflow-hidden transition-all duration-300 px-2">
                     <div class="space-y-1 py-2">
                            ...
                            <a href="news_editor.php" class="flex items-center px-4 py-2.5 text-gray-300 hover:bg-gray-700 rounded-lg transition hover:text-white text-sm">
                                <svg class="w-4 h-4 mr-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9m0 0a2 2 0 01-2-2V3a2 2 0 012-2h4a2 2 0 012 2v2a2 2 0 01-2 2z"/>
                                </svg>
                                News Page
                            </a>
                        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Backup & Maintenance</h1>
                    </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            <?php if ($message): ?>
                <div id="toast-<?= $message_type ?>" class="toast-notification bg-<?= $message_type === 'success' ? 'green' : 'red' ?>-50 border-l-4 border-<?= $message_type === 'success' ? 'green' : 'red' ?>-500 text-<?= $message_type === 'success' ? 'green' : 'red' ?>-800 p-4 rounded-lg shadow-xl flex items-center mb-4">
                    <div class="flex-shrink-0 w-10 h-10 bg-<?= $message_type === 'success' ? 'green' : 'red' ?>-100 rounded-full flex items-center justify-center mr-3">
                        <svg class="w-6 h-6 text-<?= $message_type === 'success' ? 'green' : 'red' ?>-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?= $message_type === 'success' ? 'M5 13l4 4L19 7' : 'M6 18L18 6M6 6l12 12' ?>"/>
                        </svg>
                    </div>
                    <div class="flex-1"><p class="font-semibold"><?= ucfirst($message_type) ?>!</p><p class="text-sm"><?= $message ?></p></div>
            ...
            <div class="cards-container max-w-7xl mx-auto">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Database Statistics</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div class="text-center p-4 bg-blue-50 rounded-lg">
                            <p class="text-sm text-gray-600">Total Quotes</p>
                            <p class="text-2xl font-bold text-blue-600"><?= number_format($stats['quotes']) ?></p>
                        </div>
                        ...
                        <div class="text-center p-4 bg-orange-50 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-orange-600"><?= number_format($stats['logs']) ?></p>
                        </div>
                        <div class="text-center p-4 bg-purple-50 rounded-lg">
                            <p class="text-sm text-gray-600">Database Size</p>
                            <p class="text-2xl font-bold text-purple-600"><?= $stats['db_size'] ?> MB</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Actions -->
                <div class="bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Quick Actions</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <form method="POST" class="block">
                            <input type="hidden" name="action" value="backup">
                            <button type="submit" class="w-full p-4 bg-blue-50 hover:bg-blue-100 rounded-xl text-center transition">
                                <svg class="w-10 h-10 text-blue-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                                </svg>
                                <p class="font-semibold text-blue-800">Create Backup</p>
                            </button>
                        </form>

                        <form method="POST" class="block">
                            <input type="hidden" name="action" value="optimize">
                            <button type="submit" class="w-full p-4 bg-green-50 hover:bg-green-100 rounded-xl text-center transition">
                                <svg class="w-10 h-10 text-green-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                                </svg>
                                <p class="font-semibold text-green-800">Optimize DB</p>
                            </button>
                        </form>

                        <button onclick="openCleanLogsModal()" class="w-full p-4 bg-yellow-50 hover:bg-yellow-100 rounded-xl text-center transition">
                            <svg class="w-10 h-10 text-yellow-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            <p class="font-semibold text-yellow-800">Clean Logs</p>
                        </button>
                    </div>
                </div>

                <!-- Backup History -->
                <div class="bg-white rounded-2xl shadow-xl p-6 col-span-1 lg:col-span-2">
                    <h3 class="text-xl font-bold text-gray-800 mb-6 border-b pb-3">Backup History</h3>
                    <?php if (empty($backups)): ?>
                        <p class="text-center text-gray-500 py-8">No backups found. Create your first backup!</p>
                    <?php else: ?>
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-4 border rounded-lg hover:shadow-md">
                                <div class="flex items-center gap-3">
                                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-1.657 1.657m-2.828-2.828L12 8.657"/>
                                    </svg>
                                    <div>
                                        <p class="font-medium text-gray-800"><?= htmlspecialchars($b['name']) ?></p>
                                        <p class="text-xs text-gray-600"><?= date('M d, Y H:i:s', $b['date']) ?> • <?= number_format($b['size'] / 1024 / 1024, 2) ?> MB</p>
                                    </div>
                                </div>
                                <div class="flex gap-2">
                                    <a href="../backups/<?= urlencode($b['name']) ?>" download class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                                        </svg>
                                    </a>
                                    <button onclick="openDeleteModal('<?= addslashes($b['name']) ?>')" class="p-2 text-red-600 hover:bg-red-50 rounded-lg">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </button>
